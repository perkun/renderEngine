...
Cuda::Cuda(GLuint &tex, int w, int h) {
	// jezeli w i h nie są liczbą typu 2^n to wypuść błąd
	if ( log2((float) w) != floor( log2((float) w) )  )
		printf("wymiary obrazu musza byc typu 2**n \n");

	if ( log2((float) h) != floor(log2((float) h)) )
		printf("wymiary obrazu musza byc typu 2**n \n");
